    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-6xl px-4 py-12">
        ...
        {stats && (
          <div className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
            <div className="border-4 border-black bg-cyan-300 p-6">
              <div className="mb-2 font-mono text-5xl font-black">{stats.pollsCreated}</div>
              <div className="font-mono text-lg font-bold uppercase">Polls Created</div>
            </div>
            <div className="border-4 border-black bg-yellow-300 p-6">
              <div className="mb-2 font-mono text-5xl font-black">{stats.pollsVoted}</div>
              <div className="font-mono text-lg font-bold uppercase">Polls Voted</div>
            </div>
            <div className="border-4 border-black bg-pink-300 p-6">
              <div className="mb-2 font-mono text-5xl font-black">{stats.avgVotesPerPoll}</div>
              <div className="font-mono text-lg font-bold uppercase">Avg Votes/Poll</div>
            </div>
          </div>
        )}
        ...
        <section className="mb-12">
          <h2 className="mb-6 border-4 border-black bg-lime-300 p-4 font-mono text-2xl font-black uppercase">
            My Polls
          </h2>
          {myPolls.length === 0 ? (
            <div className="border-4 border-black bg-gray-100 p-8 text-center">
              <p className="font-mono text-lg font-bold uppercase">No polls created yet</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {myPolls.map((poll) => (
                ...
                >
                  <h3 className="mb-4 font-mono text-xl font-black uppercase leading-tight">
                    {poll.question}
                  </h3>
                  <div className="border-2 border-black bg-orange-300 px-3 py-2 font-mono text-sm font-bold">
        ...
        {/* My Votes Section */}
        <section>
          <h2 className="mb-6 border-4 border-black bg-cyan-300 p-4 font-mono text-2xl font-black uppercase">
            My Votes
          </h2>
          {votedPolls.length === 0 ? (
            <div className="border-4 border-black bg-gray-100 p-8 text-center">
              <p className="font-mono text-lg font-bold uppercase">No votes cast yet</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {votedPolls.map((poll) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="border-2 border-black bg-lime-300 px-3 py-2 font-mono text-sm font-bold">
                      You voted: {poll.selectedOption}
                    </span>
                    <span className="border-2 border-black bg-gray-200 px-3 py-2 font-mono text-sm font-bold">
